...
pub fn resolve_by_sa(n_of_people: u8, n_of_group: u8, n_of_times: u8) -> String {
    ...
    //重複回数保存用の配列
    let mut duplication: DuplicatinCountVec =
    ...
    //乱数シード設定 unixTimeなので2038年問題引っかかりそう？
    let mut rng = rand_xoshiro::Xoshiro256StarStar::seed_from_u64(SystemTime::now() as u64);
    ...
    while temp > end_temp {
        ...
        //n番目の回数を入れ変える
        let mut cn: u8 = rng.gen_range(0..n_of_times);
        //iグループ目の
        let mut ci: u8 = rng.gen_range(0..n_of_group);
        //jを
        let mut cj: u8 = rng.gen_range(0..n_of_per_group);
        //kグループ目の
        let mut ck: u8 = rng.gen_range(0..n_of_group);
        //lと入れ替える
        let mut cl: u8 = rng.gen_range(0..n_of_per_group);
        ...
        //入れ替え後の重複度の計算
        update_duplication_array(&mut duplication, better_mtg, n_of_people);
        let mut after_multiplicity = calc_multiplicity(duplication, n_of_people);

        let mut diff = better_multiplicity - after_multiplicity;
...
fn update_duplication_array(
    store_array: &mut DuplicatinCountVec,
...
mod tests {
    ...
    fn test_update_duplication_array() {
        ...
        let mut test_store_array: DuplicatinCountVec =
            vec![vec![0; n_of_people as usize]; n_of_people as usize];
        ...
        println!("aaa{:?}", test_store_array);
        update_duplication_array(&mut test_store_array, test_search_array, n_of_people);
        assert_eq!(
            vec![
